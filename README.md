...
│   ├── crawl-site.js                                 // (F1) Playwright 기반 사이트 로그인 및 데이터 크롤링
│   ├── fetch-sheet.js                                // (F2) API 연동을 통한 구글 시트 데이터 읽기
│   ├── transform-data.js                             // (F3) 데이터를 요구 수식/형식에 맞춰 변환 파싱
│   ├── discord.js                                    // (F4) 디스코드 Webhook 메시지 조립 및 발송 
│   └── errors.js                                     // (F5) 커스텀 에러 정의 (발생 구역 상세 추적용)
